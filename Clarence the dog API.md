...
To keep Clarence safe and your requests secure, Pleo supports two authentication methods:
...
1. OAuth2.0 
    ...
    Best for secure, user-based access across Pleo services.
...
Ask Clarence to sit. You can specify how many times, for how long, and what reward he gets.
...
| `duration`   | String  | No       | How long Clarence should sit. Accepts `"60 secs"`, `"1 min"`, etc. Keep it short and sweet — up to 10 characters|
| `rewardType` | String  | Yes      | What Clarence gets — a `"treat"` or a `"toy"`. Keep it short and sweet — up to 5 characters|
...
| --------------- | ------- | ----------------------------------------------------------- |
| `action`        | String  | Clarence performed the action "sit".                        |
| `count`         | Integer | How many times Clarence sat.                               |
| `requestedDuration`|String| Displays how long the client asked Clarence to sit, only if a duration is specified in the request.|
| `actualDuration`   | String |   How long Clarence actually sat.                        |
| `rewardGiven`   | Boolean |  Whether Clarence was given a reward.                        |
...
Ask Clarence to offer his paw —specify 'left' or 'right' via the query parameter.
...
| Field        | Type    | Required | Description                                                               |
| ------------ | ------- | -------- | ------------------------------------------------------------------------- |
| `count`      | Integer | No       | How many times Clarence should heel. Defaults to 1 if not set. Must be ≥ 1.|
| `duration`   | String  | No       | How long Clarence should heel. Accepts `"60 secs"`, `"1 min"`, etc. Keep it short and sweet — up to 10 characters|
| `rewardType` | String  | Yes      | What Clarence gets — a `"treat"` or a `"toy"`. Keep it short and sweet — up to 5 characters|
...
| Field           | Type    |  Description                                                 |
| --------------- | ------- | ----------------------------------------------------------- |
| `action`        | String  | Clarence performed the action "heel".              |
| `count`         | Integer | How many times Clarence sat.                               |
| `requestedDuration`|String| Displays how long the client asked Clarence to sit, only if a duration is specified in the request.|
| `actualDuration`   | String |How long Clarence actually sat.                        |
| `rewardGiven`   | Boolean |  Whether Clarence was given a reward.                        |
...
```
{
  "status": 400,
  "meaning": "Bad request",
  "message": "Clarence is asleep. Please try again later when Clarence is awake."
}
```
...
| 400         | Bad request  |`Clarence is asleep.`                       | He is tired.                                         | Please try again later when Clarence is awake.  |
...
- If Clarence is asleep, any action endpoint (/sit, /paw, /heel) will return this error: HTTP Status: 400 Bad Request
